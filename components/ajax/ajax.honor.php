...
$wsdb_info = parse_worlddb_info($realm['dbinfo']);
$WSDB = DbSimple_Generic::connect("".$config['db_type']."://".$wsdb_info['user'].":".$wsdb_info['password']."@".$wsdb_info['host']."/".$wsdb_info['db']."");
...
$charinfo_arr = array();
$allhonor = array();
$charinfo_arr = $WSDB->select("SELECT character.guid,character.data,character.name,character.race,character.class FROM `character` WHERE guid IN(?a)",array_keys($honor_arr));
...
function parse_worlddb_info($str){
    $arr = explode(';',$str);
    $mangosd_database['host'] = $arr[0];
    $mangosd_database['port'] = $arr[1];
    $mangosd_database['user'] = $arr[2];
    $mangosd_database['password'] = $arr[3];
    $mangosd_database['db'] = $arr[4];
    return $mangosd_database;
}
