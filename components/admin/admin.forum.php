...
if(!$_GET['action']){
    ...
    redirect($_SERVER['HTTP_REFERER'],1);
}elseif($_GET['action']=='updcategories'){
    foreach($_POST['catorder'] as $cid=>$order){
        $DB->query("UPDATE f_categories SET cat_disp_position=?d WHERE cat_id=?d LIMIT 1",$order,$cid);
    }
    redirect($_SERVER['HTTP_REFERER'],1);
    ...
    redirect($_SERVER['HTTP_REFERER'],1);
}elseif($_GET['action']=='clearforum'){    
    clear_forum($_GET['forum_id']);
    recount($_GET['forum_id']);
    redirect($_SERVER['HTTP_REFERER'],1);
...
function moveup($cat_id,$forum_id=0){
    global $DB;
    if($forum_id>0){
        $cur_pos = $DB->selectCell("SELECT disp_position FROM f_forums WHERE forum_id=?d AND cat_id=?d LIMIT 1",$forum_id,$cat_id);      
        $target_pos = $DB->selectRow("SELECT * FROM f_forums WHERE disp_position<?d AND cat_id=?d ORDER BY disp_position DESC LIMIT 1",$cur_pos,$cat_id);
...
function movedown($cat_id,$forum_id=0){
    ...
    if($forum_id>0){
        $cur_pos = $DB->selectCell("SELECT disp_position FROM f_forums WHERE forum_id=?d AND cat_id=?d LIMIT 1",$forum_id,$cat_id);
        $target_pos = $DB->selectRow("SELECT * FROM f_forums WHERE disp_position>?d AND cat_id=?d ORDER BY disp_position ASC LIMIT 1",$cur_pos,$cat_id);
        $DB->query("UPDATE f_forums SET disp_position=?d WHERE forum_id=?d LIMIT 1",$target_pos['disp_position'],$forum_id);
        $DB->query("UPDATE f_forums SET disp_position=?d WHERE forum_id=?d LIMIT 1",$cur_pos,$target_pos['forum_id']);
    }else{
        $cur_pos = $DB->selectCell("SELECT cat_disp_position FROM f_categories WHERE cat_id=?d",$cat_id);
...
function clear_forum($forum_id){
  global $DB;
  $forum_topics = $DB->selectCol("SELECT topic_id FROM f_topics WHERE forum_id=?d",$forum_id);
  $DB->query("DELETE FROM f_posts WHERE topic_id IN (?a)",$forum_topics);
  $DB->query("DELETE FROM f_topics WHERE forum_id=?d",$forum_id);
  $DB->query("DELETE FROM f_markread WHERE marker_forum_id=?d",$forum_id);
}
?>
