...
if($_POST['step']==3){
  if($allow_reg === true){
    if($auth->register(array('username'=>$_POST['r_login'],'password'=>$_POST['r_pass'],'password2'=>$_POST['r_cpass'],'email'=>$_POST['r_email']))===true){
      if((bool)$config['req_reg_key'] === true)$auth->delete_key($_POST['r_key']);
      if((bool)$config['req_reg_act']!==true)$auth->login(array('username'=>$_POST['r_login'],'password'=>$_POST['r_pass']));
      $reg_succ = true;
    }else{
