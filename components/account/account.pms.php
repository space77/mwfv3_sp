...
if($_GET['action']=='view' && $_GET['dir']=='in'){
    ...
    $items = $DB->select("SELECT `pms`.*, `account`.`username` AS `sender` FROM `pms` 
                          LEFT JOIN `account` ON `pms`.`sender_id`=`account`.`id` 
                          WHERE `owner_id`=?d ORDER BY `posted` DESC LIMIT ?d, ?d",$user['id'],$limit_start,$items_per_pages);
    ...
    */
    ...
    //===== Calc pages =====//
    $items_per_pages = 16;
    $itemnum = $DB->selectCell("SELECT count(*) FROM pms WHERE sender_id=?d",$user['id']);
    $pnum = ceil($itemnum/$items_per_pages);
    $limit_start = ($p-1)*$items_per_pages;
    $items = $DB->select("SELECT `pms`.*, `account`.`username` AS `owner` FROM `pms`
                          LEFT JOIN `account` ON `pms`.`owner_id`=`account`.`id` 
                          WHERE `sender_id`=?d ORDER BY `posted` DESC LIMIT ?d, ?d",$user['id'],$limit_start,$items_per_pages);
    
    //echo $itemnum; exit;
}elseif($_GET['action']=='viewpm' && $_GET['iid']){
    $pathway_info[] = array('title'=>$lang['post_view'],'link'=>'');
    $item = $DB->selectRow("SELECT * FROM pms WHERE (((owner_id=?d ) OR (`sender_id`=?d)) AND (id=?d)) LIMIT 1",$user['id'],$user['id'],$_GET['iid']);
    $senderinfo = $auth->getprofile($item['sender_id']);
    if ($senderinfo['avatar']=='') {
      $senderinfo['avatar'] = 'NoAvator.gif';
    } else {
      if(!file_exists($config['avatar_path'].$senderinfo['avatar'])){
        $senderinfo['avatar'] = 'NoAvator.gif';
      }
    }
