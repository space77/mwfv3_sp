...
class AUTH {
    ...
    function check()
    {
        if(isset($_COOKIE[$this->config['site_cookie']])){
            ...
            $res = $this->DB->selectRow("
                ...
                WHERE account.id = ?d", $cookie['user_id']);
			
			$ip = $_SERVER['REMOTE_ADDR'];
      $res3 = $this->DB->selectRow("
      	SELECT `ip`, `bandate`, `unbandate` FROM `ip_banned` WHERE `ip`= ?
      ", $ip);
      	      
      if ($res3['ip']==$ip){
      	output_message('alert',"You is currently banned");
      	return false;
    	}
			      	
			$res2 = $this->DB->selectRow("
				SELECT `active` FROM account_banned WHERE id = ?d AND active = 1", $res['id']);
            if($res2['active'] > 0){
                $this->setgroup();
                $this->logout();
    ...
    */
    ...
    function gethash($str=false){
        //if($str)return sha1(utf8_encode($str)); // Returns 40 char hash.
        if($str)return sha1($str); // Returns 40 char hash.
        else return false;
    }
    ...
    function onlinelist_add() // Add or update list with new user
    {
        ...
        if($result>0)
        {
            $this->DB->query("UPDATE `online` SET `user_ip`=?,`logged`=?,`currenturl`=? WHERE `user_id`=? LIMIT 1",$this->user['ip'],$cur_time,$_SERVER['REQUEST_URI'],$this->user['id']);
        }
        else
        {
            $this->DB->query("INSERT INTO `online` (`user_id`,`user_name`,`user_ip`,`logged`,`currenturl`) VALUES (?,?,?,?,?)",$this->user['id'],$this->user['username'],$this->user['ip'],$cur_time,$_SERVER['REQUEST_URI']);
        }
    }
    ...
    function onlinelist_addguest() // Add or update list with new guest
    {
        ...
        if($result>0)
        {
            $this->DB->query("UPDATE `online` SET `user_ip`=?,`logged`=?,`currenturl`=? WHERE `user_id`='0' AND `user_ip`=? LIMIT 1",$this->user['ip'],$cur_time,$_SERVER['REQUEST_URI'],$this->user['ip']);
        }
        else
        {
            $this->DB->query("INSERT INTO `online` (`user_ip`,`logged`,`currenturl`) VALUES (?,?,?)",$this->user['ip'],$cur_time,$_SERVER['REQUEST_URI']);
        }
    }
